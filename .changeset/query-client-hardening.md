...
- Require every RPC procedure to use `asRpcQuery(...)` or `asRpcMutation(...)`, preserve that intent through fluent RPC/group composition, and reject hostile, reserved, or prefix-colliding tags before creating helpers.
...
- Retain one scoped RPC client per Angular injector, forward TanStack cancellation, return typed stream failures, and restrict factory defaults to options that are safe across heterogeneous procedures.
- Canonically encode query inputs, preserve TanStack `DataTag` and `select` inference, and make `exact: true` filters match one procedure path across all of its inputs.
